    ...
    while (window.isOpen()) //Main Game Loop
    {
        ...
        while (window.pollEvent(event))
        {
            ...
            if(event.type == sf::Event::KeyPressed && event.key.code == sf::Keyboard::A) 
            {
                ...
                player.move(-mov_speed, 0);
                window.draw(player);
                cout<<player.getPlayerChunkPos(player.getPosition()).x<<endl;
                cout<<player.getPlayerChunkPos(player.getPosition()).y<<endl;
                cout<<" "<<endl;
